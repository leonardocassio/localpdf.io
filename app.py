...
def compress_pdf(file, temp_dir, task_id=None, level="ebook"):
    ...
    #   screen  = 72 dpi  - compressao maxima
    #   ebook   = 150 dpi - balanceada
    #   printer = 300 dpi - leve
    LEVEL_DPI = {"screen": 72, "ebook": 150, "printer": 300}
    LEVEL_NAMES = {
        "screen":  "Maxima (72dpi)",
        "ebook":   "Balanceada (150dpi)",
        "printer": "Leve (300dpi)",
    }
    dpi  = LEVEL_DPI.get(level, 150)
    ...
    if task_id:
        set_progress(task_id, 20, f"Compressao {name} - preparando...")
    ...
    gs_args = [
        ...
        # Forca resampling em TODAS as imagens (sem threshold)
        "-dColorImageDownsampleThreshold=1.0",
        "-dGrayImageDownsampleThreshold=1.0",
        "-dMonoImageDownsampleThreshold=1.0",
        # DPI alvo para cada tipo
        f"-dColorImageResolution={dpi}",
        f"-dGrayImageResolution={dpi}",
        f"-dMonoImageResolution={dpi}",
        # Forca re-encode JPEG (em vez de manter codec original)
        "-dAutoFilterColorImages=false",
        "-dAutoFilterGrayImages=false",
        "-dColorImageFilter=/DCTEncode",
        "-dGrayImageFilter=/DCTEncode",
        # Qualidade JPEG: 30=screen 50=ebook 75=printer
        f"-dJPEGQ={30 if level == 'screen' else 50 if level == 'ebook' else 75}",
        # Remove metadados desnecessarios
        "-dFastWebView=false",
        "-dCompressPages=true",
        "-sDEVICE=pdfwrite",
    ...
    if task_id:
        set_progress(task_id, 40, f"Comprimindo com Ghostscript ({name})...")
